#####################
#metatranscriptomics figure4 d-e
#####################

htseq_count <- read.csv("./mi_cheese/data/T0/metatranscriptomics/3_count_tables/htseq_count_minus3053.csv")
...
genomes<-htseq_count[,1]
htseq_count<-apply(as.matrix(htseq_count[,-1]), 2, as.numeric)
rownames(htseq_count)<-genomes


names_genes_ord<-names_genes[match(rownames(htseq_count),names_genes)]
w_genes_ord<-genes@ranges@width[match(rownames(htseq_count),names_genes)]
...
colnames(htseq_count)<-gsub("sample_||_mappedReads","",colnames(htseq_count))

htseq_count<-htseq_count[,sort(colnames(htseq_count))]
...
htseq_count_scalingFactor <- apply(htseq_count, 2, function(x)sum(x)/1000000)
...
OG1313_data<-htseq_count_TPM[rownames(htseq_count_TPM)%in%OG1313_ids[-1],]
ST_data<-htseq_count_TPM[rownames(htseq_count_TPM)%in%"NLANBGKA_00526",]
data_Glutamine<-rbind(OG1313_data,ST_data)
rownames(data_Glutamine)[nrow(data_Glutamine)]<-rownames(htseq_count_TPM)[rownames(htseq_count_TPM)%in%"NLANBGKA_00526"]
...
gs
# ggsave("/PLOTS/Glutamine_synthetase.pdf",gs, units="in", width=10, height=6, dpi=600)
...
OG1318_data<-htseq_count_TPM[rownames(htseq_count_TPM)%in%OG1318_ids[-1],]
ST_data<-htseq_count_TPM[rownames(htseq_count_TPM)%in%"NLANBGKA_00527",]
data_GlnR<-rbind(OG1318_data,ST_data)
rownames(data_GlnR)[nrow(data_GlnR)]<-rownames(htseq_count_TPM)[rownames(htseq_count_TPM)%in%"NLANBGKA_00527"]
...
g_tr<-ggplot(data_GlnR,aes(x=clean_ids_names,y=value,fill=clean_ids_names))+geom_boxplot(position=position_dodge(1))+ theme_bw()+
  ...
  theme(text = element_text(size=size_all),
        ...
        legend.text = element_text(size=size_all))+facet_wrap(~condition)+scale_fill_manual(values = colors_t0s)
g_tr
# ggsave("/PLOTS/Glutamine_tr.pdf",g_tr, units="in", width=10, height=6, dpi=600)
...
prow
# ggsave("/PLOTS/Glutamine.pdf",prow, units="in", width=10, height=14, dpi=600)
