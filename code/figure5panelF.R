# Cheese paper figure 5 panel f

# load libs
library(tidyverse)

# load data, join metabolite names and species taxonomy with extracellular fluxes
bigg_rxns=read.delim("./mi_cheese/data/Models/data/bigg_models_reactions.txt")
species = read.delim("./mi_cheese/data/Models/data/species_names.tsv",header = TRUE)
fba=read.delim("./mi_cheese/data/Models/data/summary_fba.tsv",header=FALSE) %>% dplyr::rename(.,model=V1,gf_media=V2,sim_media=V3,rxn=V4,flux=V5)
...
# ggsave("Git/mi_cheese/PLOTS/FINAL_4main/fig4f.pdf",height = 5,width = 25)
