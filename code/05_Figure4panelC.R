...
# set wd
setwd("~/Documents/GitHub/mi_cheese/gem_plots")
...
milk_exp = read.delim("data/interactions.tsv") %>% mutate(compound=gsub("M_","",compound)) %>% mutate(compound=gsub("_e","",compound))
smetana_species = read.delim("data/smetana_taxonomy.tsv",header = TRUE)
metnames=read.delim("data/bigg_classes.tsv",header=TRUE) %>% dplyr::rename(.,compound=reaction)
